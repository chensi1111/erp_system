...
function SizeDocument() {
  ...
  const getList = async () => {
    try {
      const res = await axios.post('/api/size/list',{page,pageSize:10,filter,sort});
  ...
  return (
    <div className={style.container}>
      {openCreate && <CreateSizeDocument 
      onClose={() => setOpenCreate(false)} 
      onSuccess={() => {
        setOpenCreate(false);
        getList(); 
      }}/>}
      {openShow && <ShowSizeDocument 
      ...
      <div className={style.topContainer}>
        ...
        <div className={style.button} onClick={()=>setOpenCreate(true)}><FaPlus/>新增尺寸</div>
      </div>
      <div className={style.searchContainer}>
        ...
        {searchType==='size_id' && <input type="text" placeholder="搜尋關鍵字" value={filter.size_id} className={style.searchInput} onChange={(e)=>handleSetFilter(e.target.value)}/>}
        {searchType==='size_name' && <input type="text" placeholder="搜尋關鍵字" value={filter.size_name} className={style.searchInput} onChange={(e)=>handleSetFilter(e.target.value)}/>}
      </div>
            ...
            {data.map((m) => (
              <tr key={m.size_id}>
                ...
                <td className={style.actions}>
                  <button className={style.detailBtn} onClick={() => getDetail(m.size_id,false)}>
                    詳細
                  </button>
                  <button className={style.editBtn} onClick={() => getDetail(m.size_id,true)}>
                    編輯
                  </button>
                  <button className={style.deleteBtn} onClick={()=>handleDelete(m.size_id)}>
                    刪除
                  </button>
                </td>
              </tr>
