...
function ManufactorDocument() {
  ...
  const getList = async () => {
    try {
      const res = await axios.post("/api/manufactor/list", {
        page,
        pageSize: 10,
        filter,
        sort,
    ...
    try {
      ...
      if (res.data.code === "000") {
        ...
        const updateData = data.filter(
          (item) => item.manufactor_id !== manufactor_id
        );
  ...
  return (
    <div className={style.container}>
      ...
      <div className={style.searchContainer}>
        ...
        {searchType === "manufactor_id" && (
          <input
            type="text"
            placeholder="搜尋關鍵字"
            value={filter.manufactor_id}
            className={style.searchInput}
            onChange={(e) => handleSetFilter(e.target.value)}
          />
        )}
        {searchType === "manufactor_name" && (
          <input
            type="text"
            placeholder="搜尋關鍵字"
            value={filter.manufactor_name}
            className={style.searchInput}
            onChange={(e) => handleSetFilter(e.target.value)}
          />
        )}
              ...
              <tr key={m.manufactor_id}>
                ...
                <td className={style.actions}>
                  <button
                    ...
                    onClick={() => getDetail(m.manufactor_id, false)}
                  >
                    詳細
                  </button>
                  <button
                    className={style.editBtn}
                    onClick={() => getDetail(m.manufactor_id, true)}
                  >
                    編輯
                  </button>
                  <button
                    className={style.deleteBtn}
                    onClick={() => handleDelete(m.manufactor_id)}
                  >
                    刪除
                  </button>
                </td>
