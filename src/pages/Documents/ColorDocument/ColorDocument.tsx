...
function ColorDocument() {
  ...
  const getList = async () => {
    try {
      const res = await axios.post("/api/color/list", {
        page,
        pageSize: 10,
        filter,
        sort,
  ...
  return (
    <div className={style.container}>
      ...
      <div className={style.searchContainer}>
        ...
        {searchType === "color_name" && (
          <input
            type="text"
            placeholder="搜尋關鍵字"
            value={filter.color_name}
            className={style.searchInput}
            onChange={(e) => handleSetFilter(e.target.value)}
          />
        )}
              ...
              <tr key={m.color_id}>
                ...
                <td className={style.actions}>
                  <button
                    ...
                    onClick={() => getDetail(m.color_id, false)}
                  >
                    詳細
                  </button>
                  <button
                    className={style.editBtn}
                    onClick={() => getDetail(m.color_id, true)}
                  >
                    編輯
                  </button>
                  <button
                    className={style.deleteBtn}
                    onClick={() => handleDelete(m.color_id)}
                  >
                    刪除
                  </button>
                </td>
