...
function StockSafe() {
  ...
  const getList = async () => {
    try {
      const res = await axios.post("/api/stock/safe_list", {
        page,
        pageSize: 10,
        filter,
        sort,
  ...
  return (
    <div className={style.container}>
      {openShow && (
        <ShowStockSafe onClose={() => setOpenShow(false)} detail={detail} />
      )}
      <div className={style.topContainer}>
        <div className={style.title}>庫存不足列表</div>
      </div>
